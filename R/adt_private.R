...
#' Add suffix to corresponding biomarker
#'
#' @param dat A data frame that contains all the information for 
#'     variables of specific biomarkers.
#' @param suff A string indicating the type of biomarker (i.e. _cog).
#'
#' @return A data frame that contains all the information for 
#'     variables of specific biomarkers with suffix appended to each
#'     variable.
#' @export
a_add_suffix <- function(dat, suff) {
    n1 <- which(names(dat)=="subject_id")
    n2 <- which(names(dat)==paste0("date", suff))
    n_names <- as.list(paste0(names(dat)[-c(n1, n2)], suff))
    n_names <- append(n_names, "subject_id", after = n1-1)
    n_names <- append(n_names, "date", after = n2-1)
    n_names <- append(n_names, paste0("indicator", suff))
    n_names <- array(unlist(n_names))
    n_ind <- paste0("indicator", suff)
    dat %>% mutate(ind = 1) %>% 
        setNames(n_names)
}
...
#'
#' @param visit The reference time with patients' ID 
#' @param dat A data frame that contains all the information for 
#'     variables of specific biomarkers.
#' @param win A integer (unit of days) indicating the maximum 
#'     acceptable gap time for merging a biomarker test to base data.
#' @param window_overlap A logical value indicating whether the 
#'     time window should overlap.
#'
#' @return A data frame with biomarkers for each patient in every reference time.
#' @export
a_select_bmk <- function(visit, dat, win, window_overlap) {
    g_win <- function(d1, d2, left, window_overlap) {
        if (window_overlap) {
            d <- win / 2
        } else {
            d <- min(as.numeric(abs(d2 - d1)),
                     win / 2,
                     na.rm = T)
        }
        d1 + left * d
    ...
    dat_visit <- visit %>% 
        group_by(subject_id) %>% 
        mutate(date_left  = date - (date - lag(date)) / 2,
               date_right = lead(date_left)) %>% 
        rowwise() %>%
        mutate(date_left  = g_win(date, date_left,  -1, window_overlap),
               date_right = g_win(date, date_right,  1, window_overlap)) %>% 
...
#' Cutting continuous variables to categorical variables
#'
...
#'
#' @examples
a_cut_variable <- function(dat, var, n) { ## use the cut function
    dat$cate <- cut(unlist(dat[, var]), n)
    ...
}
